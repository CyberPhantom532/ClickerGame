...
function updateAutoDelayDisplay() {
  document.getElementById("auto-delay").innerText = `Auto Click Delay: ${format(Math.round(autoDelay * 100) / 100)}ms`;
}
...
function displayInfo(info, color) {
  ...
  // Makes sure color is a value and not undefined
...
function addBid() {
  ...
  const bidsList = document.getElementById("bids");
  const bidElement = document.createElement("li");
  let upperRange = Math.floor(userBid * 1.05);
...
function startGolden() {
  ...
  updateGoldTimer();
  setInterval(updateGoldTimer, 1000);
}
...
function loadGameData() {
  ...
  if (savedData) {
    ...
    if (goldRushes > 1) {
      document.getElementById("moneyFromGolden").innerText = "Money from gold rushes: " + (totalMondeyFromGolden);
    } else {
      document.getElementById("moneyFromGolden").innerText = "Money from gold rush: " + (totalMondeyFromGolden);
    }
